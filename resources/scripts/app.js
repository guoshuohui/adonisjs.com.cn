...
Zepto(function () {
	...
	if ($('#version-switch')) {
		$('#version-switch').on('change', function () {
			...
			// const curUrl = currentUrl.replace(/\/docs\/\d\.\d/, `/docs/${selectedVersion}`)
			// console.log(curUrl)
			window.location.href = `/docs/${selectedVersion}/installation`
		})
	}
	...
	if (location.pathname.indexOf('docs/3.2') > -1) {
		$('.notice3').show();
	}
})
